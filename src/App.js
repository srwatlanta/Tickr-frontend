...
class App extends Component {
      ...
      .then(data => {
        this.setState(
          prevState => ({
            ...
            currentPortfolio: prevState.currentPortfolio
              ? prevState.currentPortfolio
              : data.user.portfolios[0]
          }),
  ...
  fetchNewsData = query => {
    fetch(
      "https://newsapi.org/v2/everything?q=" +
        query +
        "&from=2019-09-10&sortBy=publishedAt&language=en&apiKey=" +
        newsAPIKEY
    )
      .then(res => res.json())
      .then(news =>
  ...
  fetchSearchStockData = stock => {
    fetch(
      `https://www.alphavantage.co/query?function=TIME_SERIES_DAILY&symbol=${stock}&apikey=${stockAPIKEY}`
    )
      .then(res => res.json())
      .then(data => {
        data["Error Message"]
          ? this.badSearch()
          : this.setSearchStockData(data);
      });
  };
  ...
  setSelectedStockInfo = data => {
    ...
    tenEntries.forEach(entry => {
      let obj = { date: entry[0], [ticker]: Number(entry[1]["4. close"]) };
      dataArr.unshift(obj);
    });
  ...
  render() {
    return (
      <Router>
        ...
        <Route
          exact
          path={`/stocks/${this.state.selectedStock.ticker}`}
          component={() => (
            <StockShowContainer
              removeSearch={this.removeSearch}
